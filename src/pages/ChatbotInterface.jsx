...
export default function ChatbotInterface({ onBack, instructionRecipe }) {
  ...
  const newlyUnlocked = (() => {
    ...
    return full.filter((a) => newOnes.includes(a.id));
  })();
  ...
  return (
    <Box sx={{ px: 2, pt: 1, pb: 1, display: "flex", flexDirection: "column", minHeight: "100%", animation: "fadeIn 0.25s ease" }}>
      ...
      {/* Complete Cooking (only when a recipe is active) */}
      ...
      {/* Input */}
      <Stack direction="row" spacing={1} sx={{ pt: 1.25, pb: 0.5, alignItems: "center" }}>
        <TextField
