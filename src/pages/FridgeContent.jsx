...
export default function FridgeContent({ onBack }) {
  const [foods, setFoods] = useLocalStorageState("ep.foods", [
    ...
    <Box sx={{ px: 2, pt: 1, pb: 3, animation: "fadeIn 0.25s ease" }}>
      ...
      >
        {sorted.map((food, index) => {
          ...
          const daysText =
            days < 0 ? `${Math.abs(days)}d ago` : days === 0 ? "Today" : days === 1 ? "Tomorrow" : `${days}d left`;
          ...
          return (
            <Box key={food.id}>
              <Stack direction="row" alignItems="center" sx={{ px: 2, py: 1.5 }}>
                ...
                <IconButton
                  size="small"
                  onClick={() => setFoods((f) => f.filter((x) => x.id !== food.id))}
                  sx={{ color: PALETTE.textTertiary, "&:hover": { color: "#B8423A" } }}
