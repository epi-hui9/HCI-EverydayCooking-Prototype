...
- **Recipes flow (two entry points):**
  ...
  - **From Start Cooking / Fridge:** Select ingredients → Energy (optional **Dietary restrictions**: No meat, No dairy) → Recipe Details (strict = selected only; good = selected + up to 2 fridge extras; if none, **Generate 3 AI recipes**) → Recipe Preview → Chat. Star any recipe to save.
...
│   ├── dietary.js            # DIETARY_OPTIONS, getExcludedIngredients (No meat, No dairy)
